(function(){
  ...
  function loadWaltonBudgetSearchModules(onReady){
    loadWcScriptOnce(
      ...
      wcBudgetAssetBaseUrl + "search-data.js?v=20260725-revenue-budget",
      function(){
        loadWcScriptOnce(
  ...
  *::after{
  ...
  nav#nav-menu .wc-search-recent-label{
    ...
    /* .42 measured 2.39:1 against the search panel -- below the 4.5:1
       minimum for this 11px bold label. Raised until it clears. */
  ...
  nav#nav-menu .wc-search-recent-pill{
    ...
    /* .68 measured 3.31:1 on the panel; the pill is an interactive control,
       so its label needs the full 4.5:1. Solid white clears it and keeps the
       chip readable against the translucent fill. */
    color:#ffffff !important;
    font-size:12px !important;
  ...
  .wc-budget-footer-links a:hover{
    background:rgba(0,63,40,0.08) !important;
  ...
  /* Restore the original six-section navigation used by the Walton County
     budget site. Search remains the final header action. */
  var WC_NAV_LINKS = [
  ...
  function ensureWcBreadcrumb(){
    ...
    if(eyebrowText === titleText){
      sectionCrumb = "";
    }else if(titleText === "Overview of Walton County" || titleText === "Organizational Structure" || titleText === "Statistical & Supplemental Information" || titleText === "Glossary, Acronyms, and Frequently Asked Questions" || titleText === "Strategic Initiatives" || titleText === "Program Budget"){
      ...
      sectionCrumb = '<a href="constitutional-officers.html">Constitutional Officers & Other Agencies</a><span class="wc-breadcrumb-sep">/</span>';
    }else if(eyebrowText === "Introduction and Overview" || eyebrowText === "Financial Structure, Policies, and Process" || eyebrowText === "Financial Plan"){
  ...
  function enhanceProgressivePages(){
    document.querySelectorAll('.wc-context-card:not(.wc-progressive-ready)').forEach(function(card){
      var header = card.querySelector(':scope > header');
      var answer = header && header.nextElementSibling;
      if(!header || !answer || answer.tagName !== 'P') return;
      ...
      body.className = 'wc-progressive-body';
      ...
      details.classList.add('wc-progressive-ready');
    ...
    if(isBoardDepartmentPage) Object.keys(departmentQuestions).forEach(function(id){
      var mount = document.getElementById(id);
      if(!mount || mount.closest('.wc-department-question')) return;
      var copy = departmentQuestions[id];
      var details = document.createElement('details');
    ...
    if(isBoardDepartmentPage){
      ...
      // Purpose-built department profiles may author a deliberate mixed
      // question order (including static and data-backed disclosures).
      // Preserve that order instead of moving the four standard mounts.
      if(orderedQuestions.length && !orderedQuestions[0].closest('.wc-omb-questions')){
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', enhanceProgressivePages, { once:true });
  }else{
    enhanceProgressivePages();
  }
