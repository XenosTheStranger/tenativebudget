...
function renderProjects(){
  ...
  // Recreation Plat Fee Fund and Sidewalk Fund don't have a Board-directed
  // project in the CIP tracker yet (see recreation-plat-fee-fund.html /
  // sidewalk-fund.html), so getFilterOptions never derives them from real
  // project rows. Shown here unconditionally so residents can find the
  // fund and see it's currently empty, matching those two fund pages.
  const fundOptions = getFilterOptions(allProjects, "funding", preferredFundOrder)
    .filter(option =>
      option.value !== normalizeFilterValue("Sheriff Fund") &&
      option.value !== normalizeFilterValue("Recreation Plat Fee Fund") &&
      option.value !== normalizeFilterValue("Sidewalk Fund")
    )
    .concat(hasSheriffProjects ? [{ label: "Sheriff Projects", value: SHERIFF_PROJECTS_FUND_VALUE }] : [])
    .concat([
      { label: "Recreation Plat Fee Fund", value: normalizeFilterValue("Recreation Plat Fee Fund") },
      { label: "Sidewalk Fund", value: normalizeFilterValue("Sidewalk Fund") }
    ]);
  ...
  app.innerHTML = `
    <style>
      ...
      *{
      ...
      .wc-intro-inner h2::after{
        ...
        border-radius:999px;
        background:linear-gradient(90deg,#003f28 0%,#0b7741 100%);
      }
      ...
      .wc-intro-divider{
        ...
        background:linear-gradient(90deg, rgba(0,63,40,0) 0%, rgba(0,63,40,0.18) 20%, rgba(0,63,40,0.28) 50%, rgba(0,63,40,0.18) 80%, rgba(0,63,40,0) 100%);
      }
      ...
      .wc-cip-list li::before{
        ...
        height:8px;
        border-radius:999px;
        background:linear-gradient(135deg,#003f28 0%,#0b7741 100%);
        box-shadow:0 0 0 3px rgba(0,63,40,0.10);
      }
      ...
      .wc-cip-data-status h2{margin:5px 0 10px;color:var(--text);font-size:clamp(26px,3vw,38px)}
      .wc-cip-data-status>p{max-width:900px;margin:0;color:var(--muted);line-height:1.7}
      .wc-capital-status-summary{display:flex;align-items:baseline;gap:10px;margin:24px 0 14px}
      .wc-capital-status-summary strong{color:var(--green);font-size:34px}
      .wc-capital-status-summary span{color:var(--muted);font-weight:800}
      .wc-capital-status-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}
      .wc-capital-status-grid div{padding:16px;border:1px solid var(--border);border-radius:14px;background:var(--light)}
      .wc-capital-status-grid span,.wc-capital-status-grid strong{display:block}
      .wc-capital-status-grid span{min-height:34px;color:var(--muted);font-size:13px;line-height:1.35}
      .wc-capital-status-grid strong{margin-top:7px;color:var(--text);font-size:18px}
      .wc-capital-status-note{margin:18px 0 0!important;padding:17px;border-left:4px solid #c9ad55;background:var(--light);color:var(--muted);font-size:14px;line-height:1.65}
      ...
      .wc-project-search:focus{
        ...
        background:#ffffff;
        box-shadow:0 0 0 4px rgba(0,63,40,0.08);
      }
      ...
      .wc-project-filter{
        ...
        padding:0 13px;
        border-radius:999px;
        border:1px solid rgba(0,63,40,0.14);
        background:#ffffff;
        color:var(--text);
        font-size:12px;
        font-weight:800;
      ...
      .wc-status-construction{ background:rgba(0,63,40,0.12); color:var(--green); }
      ...
      .wc-project-load-more{
        ...
        text-transform:uppercase;
        cursor:pointer;
        box-shadow:0 10px 24px rgba(0,63,40,0.16);
        transition:transform .22s ease, box-shadow .22s ease;
      }

      .wc-project-load-more:hover{
        transform:translateY(-2px);
        box-shadow:0 14px 28px rgba(0,63,40,0.20);
      }
      ...
      :root[data-theme="dark"] .wc-cip-plan-note,
      :root[data-theme="dark"] .wc-cip-data-status,
      ...
      @media(max-width:760px){.wc-capital-status-grid{grid-template-columns:1fr}.wc-cip-data-status{padding:22px}}
    ...
    <section class="wc-cip-main-section">
      <div class="wc-cip-main-inner">
        ...
        <aside class="wc-cip-plan-note" aria-label="How to interpret the capital plan">
          <h2>How to interpret the five-year plan</h2>
          <p>The planned project budget is the funding scheduled in this FY 2027&ndash;FY 2031 plan. It may not represent the complete life-cycle cost of a project when earlier spending, later phases, or future appropriations fall outside the five-year period.</p>
          <p>Project pages show annual funding, source, status, timing, and planning context available in the capital project record. Operational impacts are displayed when they have been provided; a blank field should not be interpreted to mean that a completed asset will have no future staffing, utility, maintenance, or replacement cost.</p>
        </aside>

        <section class="wc-cip-data-status" aria-labelledby="wc-cip-data-status-title">
          <span class="wc-cip-kicker">Capital Plan Disclosure</span>
          <h2 id="wc-cip-data-status-title">What is known&mdash;and what remains to be estimated.</h2>
          <p>This completeness review distinguishes information contained in the current project records from information still needed for full project planning and GFOA review.</p>
          <div id="gfoa-capital-disclosure"><p>Reviewing capital project records&hellip;</p></div>
        </section>
