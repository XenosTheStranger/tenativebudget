...
function renderPlanningContext(project){
  const priority = String(project.priority || "").trim();
  const values = [
    ["Project Priority", priority && priority.toLowerCase() !== "none" ? priority : ""],
    ["Strategic Goals", project.strategic_goals],
    ["Operational Impact", project.operational_impact],
    ["Additional Planning Information", project.pertinent_information]
  ].filter(item => hasDisplayValue(item[1]));
  ...
  return `
    <section class="wc-project-panel">
      <h2>Planning &amp; Operating Context</h2>
      <div class="wc-project-list">
        ${values.map(item => renderListItem(item[0], item[1])).join("")}
      </div>
    </section>
  `;
}
