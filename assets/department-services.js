(function(){
  ...
  // "View Personnel Cost Sheet": this department's own slice of the
  // Summary of Personnel Cost page's per-position dollar breakdown
  // (salary, COLA, retirement/health/other benefits, total), reusing
  // that page's exact computation via
  // window.WCBudgetData.getDepartmentPersonnelCostDetail so the two
  // never disagree.
  // The shared Summary of Personnel Cost table keeps COLA and Health
  // Insurance Increase as their own optional columns (that page has its
  // own toggle for them), but this per-department popup has no such
  // toggle, so they're folded into Salaries & Wages and into Retirement/
  // Health/Other Benefits here -- only in this popup's own copy of the
  // markup, not the shared table itself.
  function mergePersonnelCostOptionalColumns(html){
    var container=document.createElement('div');
    container.innerHTML=html;
    container.querySelectorAll('.wc-staffing-notes-title').forEach(function(node){
      if(node.textContent.trim()==='Cost by Position') node.remove();
    });
    function parseCell(text){return Number(String(text||'').replace(/[^0-9.-]/g,''))||0;}
    container.querySelectorAll('.wc-staffing-table tr').forEach(function(row){
      ...
      cells[2].textContent=money(parseCell(cells[2].textContent)+parseCell(cells[3].textContent));
      cells[4].textContent=money(parseCell(cells[4].textContent)+parseCell(cells[5].textContent));
      cells[5].remove();
      cells[3].remove();
    });
    return container.innerHTML;
  }
  function bindSnapshotPersonnelCostSheet(button,deptKey,deptLabel){
    button.addEventListener('click',function(){
      if(!window.WCBudgetData||typeof window.WCBudgetData.openBudgetDetailPanel!=='function'||typeof window.WCBudgetData.getDepartmentPersonnelCostDetail!=='function') return;
      var html=window.WCBudgetData.getDepartmentPersonnelCostDetail(deptKey);
      if(html===null||html===undefined){
        html='<div class="wc-data-empty">No personnel cost data is available for this department.</div>';
      }else{
        html=mergePersonnelCostOptionalColumns(html);
      }
      window.WCBudgetData.openBudgetDetailPanel(button,{title:'Personnel Cost Sheet',kicker:deptLabel||'',bodyClassName:'wc-snapshot-personnel-cost-body',html:html});
    });
  }
  ...
  function openSnapshotSheet(kind){
    var mountId=kind==='personnel'?'department-staffing-table':kind==='revenue'?'department-revenue-table':'department-expense-table';
    var mount=document.getElementById(mountId);
    if(!mount) return;
    var question=mount.closest('.wc-department-question');
    if(question) question.open=true;
    var sheetToggle=kind==='budget'
      ? document.querySelector('[data-profile-full-budget-toggle]')||mount.querySelector('.wc-view-budget-lines-toggle[data-target]')
      : mount.querySelector('.wc-view-budget-lines-toggle[data-target]');
    if(sheetToggle&&sheetToggle.getAttribute('aria-expanded')!=='true') sheetToggle.click();
    if(!sheetToggle) (question||mount).scrollIntoView({behavior:'smooth',block:'start'});
  }
  document.addEventListener('click',function(event){
    var button=event.target.closest('[data-profile-sheet-jump]');
    if(!button) return;
    event.preventDefault();
    openSnapshotSheet(button.dataset.profileSheetJump);
  });
  document.addEventListener('click',function(event){
  ...
  function render(){
    ...
    if(mediaWrapper){
      Array.prototype.forEach.call(mediaWrapper.children,function(child){
        ...
        if(child.matches&&child.matches('.wc-video-frame,.extension-video-frame,.mosquito-video-frame,.libraries-video-frame,.wc-omb-award-top,figure,a[class*="iframe-link"]')) supportingMedia.push(child);
      });
    }
    ...
    var personnelCostButton=snapshot.querySelector('[data-profile-personnel-cost-trigger]');
    if(personnelCostButton) bindSnapshotPersonnelCostSheet(personnelCostButton,key,title.textContent.trim());
