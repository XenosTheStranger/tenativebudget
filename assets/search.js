(function(){
  window.initWaltonBudgetSearch = function(options){
    ...
    slot.innerHTML = `
      <div class="wc-nav-search-results" role="presentation">
        <div class="wc-search-panel">
          ...
          <!-- role="listbox" sits on the element that actually owns the
               options, not on the scroll container. A listbox may only own
               option and group children, and the scroll container also holds
               the "Search Results" kicker, which made the whole listbox
               invalid to a screen reader. -->
          <div class="wc-search-scroll">
            <div class="wc-search-kicker">Search Results</div>
            <div class="wc-search-results-inner" role="listbox" aria-label="Search results"></div>
          </div>
        </div>
      </div>
    `;
    ...
    function renderResultGroup(label, items, normalizedQuery){
      // The listbox owns these wrappers directly, so each must be a role it
      // permits. "group" is the allowed container role; its accessible name
      // comes from aria-label, which leaves the visible heading redundant to
      // a screen reader and lets it be hidden from the accessibility tree.
    ...
    function renderResults(query){
      ...
      if(!matches.length){
        // role="presentation" keeps this status text from counting as a
        // malformed listbox child while it is the only thing in the box.
        resultsInner.innerHTML = '<div class="wc-nav-search-empty" role="presentation">No matching sections found.</div>';
        results.classList.add("is-active");
        return;
      }
