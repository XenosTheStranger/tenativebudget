(function(){
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .wc-split-brand-top{
        ...
        letter-spacing:var(--wc-brand-tracking, .06em) !important;
  ...
  function equalizeBrandTextWidth(block){
    ...
    top.style.removeProperty("--wc-brand-tracking");
    ...
    // Feeds a custom property that the stylesheet rule above reads, rather
    // than setting letter-spacing inline. An inline letter-spacing cannot be
    // overridden by a reader's own stylesheet at any specificity, which fails
    // WCAG 1.4.12 (text spacing) -- and accessibility compliance is scored
    // directly by GFOA. Routing through a variable keeps the two wordmark
    // lines optically equal while leaving the declaration overridable.
    top.style.setProperty("--wc-brand-tracking", nextSpacing + "px");
  }
