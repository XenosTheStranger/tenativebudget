...
(function () {
  ...
  const DEPT_ALIASES = {
    ...
    "libraries": ["county libraries", "library"],
  ...
  // Shared by the Revenue Budget' county-wide "View Budget Lines"
  ...
  // The FY2027 tentative budget for Dept_Code 101312 / Revenue_Code 312410
  ...
  // to their FY 2027 tentative budget weights (or equally if all are $0).
  //
  ...
  // isn't included/funded in the FY2027 tentative budget -- a separate
  ...
  function renderBudgetLinesToggle(rows, descriptionField, kind, combineByName, forceDisablePriorYears, currentBudgetOnly) {
    ...
    // On the Revenue Budget' county-wide list, the many individual
    ...
    if (combineByName) {
      ...
      // just noise on the Revenue Budget/Expenses "View Budget Lines"
  ...
  function departmentFundingContextHtml(rows) {
    const sources = (rows || []).map((row) => {
      const amount = revenueDisplayAmount(row.FY2027_Proposed || 0);
      const code = String(row.Revenue_Code || "").trim();
      const name = normalizeDeptName(row.Revenue_Name);
      return { amount, code, name };
    }).filter((source) => source.amount > 0);
    if (!sources.length) return "";

    const total = sources.reduce((sum, source) => sum + source.amount, 0);
    const amountFor = (predicate) => sources.reduce((sum, source) =>
      sum + (predicate(source) ? source.amount : 0), 0
    );
    const propertyTax = amountFor((source) =>
      source.code === "311000" || source.code === "311001" || source.name.indexOf("ad valorem") !== -1
    );
    const touristTax = amountFor((source) =>
      source.name.indexOf("tourist development") !== -1 || source.name.indexOf("bed tax") !== -1
    );
    const salesTax = amountFor((source) =>
      source.name.indexOf("sales tax") !== -1 ||
      source.name.indexOf("sales surtax") !== -1 ||
      source.name.indexOf("state revenue share") !== -1
    );
    const visitorLinked = touristTax + salesTax;
    if (propertyTax > 0 || visitorLinked <= 0) return "";

    const percent = total ? visitorLinked / total * 100 : 0;
    const isTouristTaxOnly = touristTax > 0 && salesTax === 0;
    const heading = isTouristTaxOnly
      ? "Tourism helps fund this service"
      : "This service is not funded by property taxes";
    const sourceLabel = isTouristTaxOnly
      ? "Tourist Development Tax paid on short-term lodging"
      : touristTax > 0
        ? "tourist-development and sales-tax revenue"
        : "state-shared or local sales-tax revenue";
    const context = isTouristTaxOnly
      ? "This tax is generally paid by visitors staying in hotels and short-term rentals."
      : "Sales taxes are paid on purchases by residents and visitors; Walton County's visitor economy contributes to this tax base.";

    return (
      '<aside class="wc-department-funding-context" aria-label="Important funding context">' +
        '<span class="wc-department-funding-context-label">Important funding context</span>' +
        '<strong>' + escapeHtml(heading) + '</strong>' +
        '<p><b>' + escapeHtml(formatCurrency(visitorLinked)) + '</b> &middot; ' +
          escapeHtml(percent.toFixed(percent >= 10 ? 0 : 1)) + '% of this department&rsquo;s listed FY 2027 revenue comes from ' +
          escapeHtml(sourceLabel) + '. ' + escapeHtml(context) + '</p>' +
      '</aside>'
    );
  }
  ...
  function renderFinancialDashboardCard(options) {
    ...
    // Department pages need to answer the GFOA cost question at first
    // glance. Show every expenditure/revenue category here; individual
    // account lines remain progressively disclosed behind the footer
    // button. The previous top-three limit hid part of the cost.
    const visibleRows = nonZeroRows;
  ...
  function renderTypeSummaryGroup(rows, kind, caption, notes, descriptionField, showChange, combinedChangeByType) {
    ...
    return renderFinancialDashboardCard({
      ...
      contextHtml: isExpense ? "" : departmentFundingContextHtml(rows)
    });
  }
  ...
  // correction, which would also change how Revenue Budget/Fund
  ...
  function buildFundFinancialSchedule(fundCodes, caption) {
    ...
    function sumFor(rows, predicate, field) {
      ...
      // Revenue Budget even after sign-correcting r[field] directly.
      ...
      return sourceRows.reduce((sum, r) => {
        ...
        // revenue totals didn't match the Revenue Budget table (see
  ...
  // "Revenue Budget" page: historical actuals (FY2020-FY2025) by
  // revenue category, live from the revenues sheet.
  function isPropertyTaxRevenueSummaryRow(row) {
    const code = String((row && row.Revenue_Code) || "").trim();
    const name = normalizeDeptName(row && row.Revenue_Name);
    return code === "311000" || code === "311001" ||
      (code === "389001" && String((row && row.Dept_Code) || "").trim() === "102389") ||
      name.indexOf("ad valorem") !== -1;
  }

  const CONSOLIDATED_REVENUE_SUMMARY_ROWS = [
    { type: "General Government Taxes", label: "Property Taxes", predicate: isPropertyTaxRevenueSummaryRow },
    { type: "General Government Taxes", label: "General Government Taxes", predicate: (row) => !isPropertyTaxRevenueSummaryRow(row) },
  ...
  function renderConsolidatedRevenueSummaryTable() {
    ...
    function rawRevenueActualSummarySum(rowsToSum, field) {
      ...
      // detail table (see the Revenue Budget vs. its own detail
    ...
    const bodyRows = CONSOLIDATED_REVENUE_SUMMARY_ROWS.map((spec) => {
      ...
      const matching = rows.filter((r) =>
        ...
        (!spec.predicate || spec.predicate(r)) &&
        !isReportedElsewhere(r)
      );
      // Mosquito Control levies its own Ad Valorem millage in addition to
      // the County's General Fund millage. Both belong in the new Property
      // Taxes category, but its rows are relabeled in the account detail so
      // readers can distinguish the Mosquito Control levy from countywide
      // Ad Valorem Taxes.
      let matchingForDetail = matching.map((r) => {
        const isMosquitoAdValorem = spec.label === "Property Taxes" &&
      ...
      const rowHtml = (
        ...
        CONSOLIDATED_REVENUE_SUMMARY_COLUMNS.map((col, i) => {
          ...
          totals[i] += sum;
          if (col.field === "FY2027_Proposed") proposedForOverview = sum;
          return '<td class="wc-num' + (i < lastIndex ? " wc-prior-year" : "") + '">' + formatCurrency(sum) + "</td>";
        }).join("") +
        "</tr>"
      );
      overviewRows.push({ label: spec.label, amount: proposedForOverview });
      return rowHtml;
    });
    ...
    const proposedTotal = totals[lastIndex] || 0;
    const overviewHtml =
      '<section class="wc-revenue-at-a-glance" aria-labelledby="revenue-at-a-glance-title">' +
        '<div class="wc-revenue-total"><span>FY 2027 tentative revenue</span><strong id="revenue-at-a-glance-title">' + escapeHtml(formatCompactCurrency(proposedTotal)) + '</strong></div>' +
        '<div class="wc-revenue-category-grid">' + overviewRows.filter((row) => row.amount !== 0).sort((a, b) => b.amount - a.amount).map((row) => {
          const share = proposedTotal ? row.amount / proposedTotal * 100 : 0;
          return '<article><div><span>' + escapeHtml(row.label) + '</span><strong>' + escapeHtml(formatCompactCurrency(row.amount)) + '</strong></div><b>' + escapeHtml(share.toFixed(share >= 10 ? 0 : 1)) + '%</b></article>';
        }).join('') + '</div>' +
      '</section>';
    const visitorTaxRows = rows.filter((r) => {
      const name = normalizeDeptName(r.Revenue_Name);
      return !isReportedElsewhere(r) &&
        (name.indexOf("tourist development tax") !== -1 || name.indexOf("bed tax") !== -1);
    });
    const identifiableVisitorRevenue = dedupedRevenueSum(visitorTaxRows, "FY2027_Proposed");
    const visitorRevenueHtml = identifiableVisitorRevenue > 0
      ? '<section class="wc-visitor-revenue-answer" id="visitor-revenue" aria-labelledby="visitor-revenue-title">' +
          '<span>Who pays</span>' +
          '<div class="wc-visitor-revenue-lead"><h2 id="visitor-revenue-title">Visitors generated $214.7 million for County government.</h2>' +
          '<p>Walton County Tourism&rsquo;s 2025 visitor study estimates <strong>$214,717,987</strong> in taxes and user-fee revenue to Walton County government attributable to visitors. After an estimated <strong>$153,817,563</strong> cost to serve visitors, the study calculates a <strong>$60,900,424 net benefit</strong>&mdash;equivalent to <strong>$1,772 per Walton County household</strong>.</p></div>' +
          '<div class="wc-visitor-revenue-metrics" aria-label="Calendar year 2025 visitor contribution">' +
            '<div><strong>68%</strong><span>of spending in Walton County was visitor spending</span></div>' +
            '<div><strong>$3.93B</strong><span>in measured visitor spending in Walton County</span></div>' +
            '<div><strong>$243.4M</strong><span>in state and local taxes generated by visitation</span></div>' +
            '<div><strong>' + escapeHtml(formatCompactCurrency(identifiableVisitorRevenue)) + '</strong><span>FY 2027 Tourist Development Tax budget</span></div>' +
          '</div>' +
          '<p class="wc-visitor-revenue-limit"><strong>How to read these figures:</strong> Visitor purchases form a substantial part of Walton County&rsquo;s sales-tax base, but the study does not publish one separate dollar figure for sales tax alone. The $243.4 million estimate includes state and local taxes generated by visitation; $214.7 million is the study&rsquo;s estimate of taxes and user fees flowing specifically to Walton County government. The FY 2027 Tourist Development Tax amount comes from the tentative budget and is shown separately because it is directly identifiable as a visitor-paid lodging tax. <a href="https://www.waltoncountyfltourism.com/userfiles/Walton_County_Tourism_2025_Annual_Visitor_Tracking_Report_2.pdf" target="_blank" rel="noopener noreferrer">Read the 2025 Annual Visitor Tracking Report</a>.</p>' +
        '</section>'
      : '';
    return overviewHtml + (
      '<details class="wc-revenue-schedule-disclosure"><summary><span>Open the complete revenue schedule<small>Historical actuals, FY 2026 budget, FY 2027 proposal, and every revenue account</small></span></summary>' +
      ...
      "</div></details>" + visitorRevenueHtml
    );
  }
  ...
  function renderExpenseDepartmentBudgetLinesFooter(rows, dedupedRows) {
    ...
    // Unlike Revenue Budget, this department-level detail has no
    ...
    // Grouped by activity/source (same layout as the Revenue Budget'
  ...
  // Charts created while their canvas sits inside a hidden tab panel or a
  // closed <details> (e.g. the revenue topic charts, which live inside the
  // Full Detail tab and a classification's <details>) get baked in at
  // zero size by Chart.js and never fix themselves on their own. Anything
  // that reveals such a container should call this afterward so those
  // charts pick up their real size.
  const wcRevealResizeCharts = [];
  function registerWcRevealResizeChart(chart, canvas) {
    wcRevealResizeCharts.push({ chart, canvas });
  }
  function resizeVisibleRevealCharts() {
    wcRevealResizeCharts.forEach(({ chart, canvas }) => {
      if (canvas.offsetParent !== null) chart.resize();
    });
  }
  window.wcResizeRevealCharts = resizeVisibleRevealCharts;
  ...
  // "Revenue Budget" page: a narrative + bar-chart card for each major
  ...
  function buildMiscellaneousRevenueTopics() {
    return [
      ...
      // row on the Revenue Budget detail table (any Revenue_Name
  ...
  function renderRevenueTopicPlaceholders(container, topics) {
    ...
    container.innerHTML = topics.map((topic) =>
      '<details class="wc-revenue-detail-group wc-revenue-topic-block" id="' + escapeHtml(revenueTopicSlug(topic.title)) + '">' +
      '<summary>' + escapeHtml(topic.title) + "</summary>" +
  ...
  function renderRevenueTopicCards(container, topics, idPrefix) {
    ...
    topics.forEach((topic, topicIndex) => {
      ...
      registerWcThemedChart(chart);
      registerWcRevealResizeChart(chart, canvas);
  ...
  function renderPerformanceTable(rows) {
    ...
    return (
      ...
      "<strong>How to read this table:</strong> The FY 2027 column is the proposed target for each measure. Prior years provide trend context when available. The department budget shown elsewhere on this page is the department&rsquo;s total resource plan and is not a direct cost allocation to an individual goal, objective, or performance measure. The code link corresponds to a Strategic Priority Initiative identified by the Walton County Board of County Commissioners." +
      "</div>" +
      "</section>"
    );
  }
  ...
  function arrangeDepartmentFinancialDashboard(expenseEl, revenueEl, staffingEl, supplementalExpenseEls, deptName) {
    // nav.js places the four standard department mounts inside separate,
    // question-led disclosures. Keep those mounts with their questions;
    // combining them into the legacy dashboard grid would move revenue and
    // staffing into the expense disclosure and leave empty controls behind.
    if (expenseEl && expenseEl.closest(".wc-department-question")) return;
  ...
  // ---- financial summary pages (Summary of Expenses / Revenue Budget) ----
  ...
  function renderMachinerySummary(container) {
    ...
    // Requested but not funded in the FY2027 tentative budget -- a small,
    ...
    container.innerHTML =
      ...
      (unfundedRows.length
        ? '<p class="wc-table-label wc-machinery-unfunded-label">Requested But Not Included in the Budget</p>' +
          // Naming the decision that produced the item closes the loop GFOA's
          // budget-process criterion asks about: a reader who lands here can
          // see not just that the request was denied but where and when.
          '<p class="wc-machinery-unfunded-note">A department asked for these items and the tentative budget does not fund them. The Environmental Services vessel and trailer was removed from the capital request on the direction of the Board of County Commissioners at the public budget workshop held on July 14, 2026. See the <a href="budget-process-and-calendar.html#wc-process-input-title">Public Input &amp; Decision Record</a>.</p>' +
  ...
  function renderPersonnelCostSummary(container) {
    ...
    container.innerHTML =
      ...
      '<button type="button" class="wc-view-budget-lines-toggle" id="wcPersonnelCostIncreasesToggle" aria-pressed="false">Show COLA &amp; Health Insurance Scenario</button>' +
    ...
    increasesToggle.addEventListener("click", () => {
      ...
      increasesToggle.textContent = showing ? "Hide COLA & Health Insurance Scenario" : "Show COLA & Health Insurance Scenario";
      ...
      mountOrHide(
        ...
        renderTable({
          ...
          columns: [
            ...
            { label: "5.5% Health Insurance Scenario", num: true, classes: ["wc-personnel-cost-optional-col"] },
  ...
  function strategicDepartmentBudget(row) {
    ...
    // the code match has no tentative budget.
  ...
  function strategicInitiativeGroupHtml(initiative, rows) {
    ...
    rows.forEach((row) => {
      ...
      if (!departments.has(key)) {
        departments.set(key, {
          code: row.Dept_Code,
          name: DEPARTMENT_PAGE_TITLE_ALIASES.get(normalizeDeptName(row.Dept_Name)) || row.Dept_Name || "Department not identified",
    ...
    return (
      '<details class="wc-alignment-initiative" id="priority-detail-' + initiative.code + '" data-initiative="' + initiative.code + '">' +
        ...
        '<div class="wc-alignment-initiative-body">' +
  ...
  function initStrategicInitiativesPage() {
    ...
    loadBudgetData().then((data) => {
      ...
      const priorityItems = groups.map(({ initiative, rows }) => {
        const departments = new Map();
        rows.forEach((row) => {
          const key = row.Dept_Code || normalizeDeptName(row.Dept_Name);
        ...
        const goalCount = countUniqueGoals(rows);
        const card = '<button type="button" class="wc-priority-card" data-initiative-filter="' + initiative.code + '" aria-controls="priority-detail-' + initiative.code + '" aria-pressed="false">' +
          ...
          goalCount + " " + (goalCount === 1 ? "goal" : "goals") + '</small></span>' +
          '<span class="wc-priority-metric"><strong>' + departments.size + '</strong><span>Departments</span></span>' +
          '<span class="wc-priority-metric"><strong>' + goalCount + '</strong><span>Goals</span></span>' +
          '<span class="wc-priority-budget"><span>FY 2027 department budgets represented</span>' + formatCurrency(budget) + "</span>" +
          '<span class="wc-priority-action">View &rarr;</span>' +
        "</button>";
        return '<div class="wc-priority-item" data-priority="' + initiative.code + '">' + card +
          strategicInitiativeGroupHtml(initiative, rows) + "</div>";
      }).join("");

      container.innerHTML =
        '<div class="wc-priority-columns" aria-hidden="true"><span></span><span>Board priority</span><span>Departments</span><span>Goals</span><span>FY 2027 budget represented</span><span></span></div>' +
        '<div class="wc-priority-grid" role="group" aria-label="Board strategic priorities">' + priorityItems + "</div>";
      ...
      function selectPriority(selected, options) {
        ...
        container.querySelectorAll(".wc-priority-card").forEach((item) => {
          ...
          const action = item.querySelector(".wc-priority-action");
          if (action) action.innerHTML = active ? "Hide &uarr;" : "View &rarr;";
        });
        ...
        if (options.scroll) {
          const target = selected === "all"
            ...
            : container.querySelector('.wc-priority-item[data-priority="' + selected + '"]');
      ...
      container.addEventListener("click", (event) => {
        ...
        if (!button) return;
        const selected = button.classList.contains("wc-priority-card") && button.classList.contains("is-active")
          ? "all"
          : button.dataset.initiativeFilter;
        selectPriority(selected, { scroll: true });
      });
      ...
      container.querySelector(".wc-priority-grid").addEventListener("toggle", (event) => {
  ...
  window.WCBudgetData = {
    ...
    // Consolidated Expenditure table uses. Deriving the mapping separately
    // would let the two presentations drift apart and disagree about which
    // function a department belongs to.
    expenseActivityForRow
  };
})();
